...
int64_t bshuf_compress_zstd_block(ioc_chain *C_ptr, \
    ...
    tmp_buf_zstd = malloc(ZSTD_compressBound(size * elem_size));
    ...
    nbytes = ZSTD_compress((char*) tmp_buf_zstd, ZSTD_compressBound(size * elem_size), (const char*) tmp_buf_bshuf, size * elem_size, ZSTD_CLEVEL_DEFAULT);
...
int64_t bshuf_decompress_zstd_block(ioc_chain *C_ptr,
    ...
    nbytes = ZSTD_decompress((char *) tmp_buf, size * elem_size, (const char*) in + 4, nbytes_from_header);
